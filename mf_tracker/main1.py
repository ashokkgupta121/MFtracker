...
# ── Data store ────────────────────────────────────────────────────────────────
DATA_FILE  = Path.home() / ".mf_tracker" / "portfolio.json"
META_FILE  = Path.home() / ".mf_tracker" / "meta.json"
DATA_FILE.parent.mkdir(parents=True, exist_ok=True)

AUTO_REFRESH_HOURS = 8   # auto-refresh if app hasn't been refreshed in this many hours
...
class MFTracker(QMainWindow):
    def __init__(self):
        ...
        self._build_ui()
        self._refresh_table()
        # Auto-refresh NAV after window is shown if data is stale
...
""")
    ...
    def _edit_fund(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            # Re-fetch NAV if purchase date changed (nav_history was cleared in dialog)
    ...
    def _auto_refresh_if_stale(self):
        if not self.portfolio:
            return
        hours = hours_since_last_refresh()
        if hours >= AUTO_REFRESH_HOURS:
            ...
            self._set_status(msg, duration=0)   # persistent until refresh done
    ...
    def _on_nav_fetched(self, code, history):
        ...
        if self._pending <= 0:
            save_portfolio(self.portfolio)
            # Record timestamp of this successful refresh
            meta = load_meta()
            meta["last_refreshed"] = datetime.datetime.now().isoformat()
            save_meta(meta)
            self._refresh_table()
